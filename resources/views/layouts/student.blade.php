...
<head>
  ...
  @stack('styles')
  <style>
    /* ---------- Design Tokens (Cupertino, matching Admin) ---------- */
    :root{
      --sidebar-w: 280px;
      --bg: #F5F5F7;
      --surface: #FFFFFF;
      ...
      --accent:#34C759; /* student accent (green) */
    }
    ...
    @media (prefers-color-scheme: dark){
      .sidebar{ background: rgba(28,28,30,.55); }
    }
    .brand{
    ...
    @media (prefers-color-scheme: dark){
      .titlebar{ background: linear-gradient(180deg, rgba(20,20,22,.9), rgba(20,20,22,.65)); }
    }
    .titlebar-inner{
    ...
    /* ---------- Main container ---------- */
    .page{ max-width: 1240px; margin: 22px auto 40px; padding: 0 20px; }
    .card-cupertino{
    ...
    .btn-logout{
      border-radius:12px; font-weight:700; border:1px solid var(--stroke);
  ...
  <aside class="sidebar" id="sidebar">
    <div class="brand">
      <div class="brand-logo">
        ...
        <i class="fa-solid fa-user-graduate brand-fallback"></i>
      </div>
      <div>
        <div class="brand-title">Student Portal</div>
        <div class="brand-sub">Borrowing System</div>
      </div>
    </div>
    ...
    <a class="nav-link {{ request()->routeIs('student.borrow') ? 'active' : '' }}" href="{{ route('student.borrow') }}">
      <i class="fa-solid fa-handshake"></i> <span>Borrow</span>
    </a>
  ...
  <div class="content">
    <header class="titlebar">
      <div class="titlebar-inner">
        <h1 class="page-title">@yield('title','Student')</h1>
