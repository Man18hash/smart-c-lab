...
<head>
  ...
  <title>Student Login • Borrowing System</title>
  ...
  <style>
    ...
    .sheet{width:min(360px, 92vw); text-align:center}
    .plane{position:absolute; right:12px; top:12px; width:80px; opacity:.5}
...
<body>
  ...
  <div class="canvas">
    <div class="sheet">
      ...
      <h1>Login</h1>
      <div class="hint">Student access</div>
      ...
      <form method="POST" action="{{ route('student.login.attempt') }}" class="d-grid gap-3 mt-3">
        @csrf
        <input class="pill" type="email" name="email" placeholder="Email" value="{{ old('email') }}" required autofocus>
        ...
        <div class="links text-center">
          Admin instead? <a href="{{ route('login') }}">Admin Login</a>
        </div>
      </form>
    </div>
  </div>
